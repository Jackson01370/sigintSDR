...
- **DCスパイク除外**は「中央集中・時間不変・細い」の3条件で判定する。中央に貼り付いた
  まま**一定**で居座る細い線（＝DCオフセット由来）を狙う。逆に言えば、本物の狭帯域信号が
  たまたま取得帯域のちょうど中央に**常時一定**で居続けると、3条件を満たして DCスパイク扱い
  になりうる（本物のBLE等はホップ/明滅するか中央からオフセットするため通常は当たらない）。
  なお、滞在中ずっと帯域を埋め続ける常時広帯域信号は元々 dwell では過小検出（上記）であり、
  サーベイ側で拾う前提。
