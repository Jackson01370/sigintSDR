...
**方式（平均値を引く）**: 取得した複素IQ全体の平均（complex mean）を各サンプルから引く
（`iq = iq - iq.mean()`、`dsp.remove_dc`）。これは信号全体に乗った定常的なDCオフセット
（=中央スパイクの主因）だけを取り除く。中央外にオフセットした信号・帯域内ノイズに対して平均は
ほぼ 0 なので、本物の信号やその占有帯域は壊さない（除去するのは 0Hz 成分のみ）。

**どこで効くか**: `sdr.HackRFBackend` が受信の**最も入口**（`_read`）で適用するため、その後の
全処理（サーベイ／ドウェル／測定／画像化／保存）が DC 除去済みの信号を見る。サーベイ
