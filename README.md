...
- **コムスプリアス除外**: 等間隔・同一強度で並ぶ細いピーク列（受信機内部由来。アンテナ無しでも
  出る固定パターン）を検出して破棄。
- **DCスパイク除外**: 取得帯域のちょうど**中央（中心周波数=オフセット0Hz）**に出る、本物の
  電波ではない一定の細い線を破棄（`--q-dc-excess` / `--q-dc-std`）。ゼロIF受信機（HackRF等）の
  DCオフセットに由来する。**「中央集中」かつ「時間不変」かつ「細い」の3条件が揃ったときのみ**
  DCスパイクとみなす。中央からオフセットした信号・広帯域信号（WiFi等）・時間変動するバースト
  （BLE等）は対象外（本物として残す）。

> **DCスパイクとは**: 実機で 2.4GHz を滞在観測し SigMF を画像化すると、ほぼ全ての画像の
> 中央に「時間方向に全く揺らがない一直線の細い線」が写る。これは受信機のDCオフセットが
> 作る偽信号で、本物のバーストのように明滅せず、中心に貼り付いたまま動かない。
> ```
>   周波数 →            中央(DC)
>   ┌───────────────────┊───────────────────┐
> 時│  ·   wifi  ·       ┃        ·   ble  ·  │  ┃ = DCスパイク（中央固定・時間不変・細い）→ 破棄
> 間│ wifi    ·   wifi   ┃   ·         ·      │  · = 本物のバースト（明滅し位置も散る）→ 残す
> ↓ │   ·  wifi   ·  ·   ┃     ·   ble        │
>   └───────────────────┊───────────────────┘
> ```
> 判定は各取得IQの**高分解能PSD**で「中央バンドのピーク − 両脇リングの中央値（= `dc_excess`）」を
> 測り、滞在中の観測間でこれが**大きく（中央集中・細い）かつほぼ一定（時間不変）**かを見る。
> 広帯域信号は両脇も上がって `dc_excess` が縮み、中央外の信号は中央が上がらず縮み、時間変動
> するバーストは観測間のばらつきが大きく外れるため、いずれも残る。サーベイ平滑後の帯域幅に
> 依存しないので、検出帯に紛れて narrow-steady-spur をすり抜けていた中央スパイクも捕まえる。

保存件数が少なくなるのは想定内。足切りを外したいときは `--no-quality-gate`。品質メタ
（観測回数・持続率・SNR統計・スプリアス疑い・`sigscan:dc_spike` 等）は SigMF の annotation に
`sigscan:` 名前空間で記録される（`global` には `sigscan:capture_mode="dwell"`）。
...
## DCスパイク除去（DC offset correction）

実機で収集・画像化すると、取得帯域の**中央（中心周波数=オフセット0Hz）に時間で揺らがない
一直線**が写り込む。これはゼロIF受信機（HackRF等）の**DCオフセット**が作る偽信号（DCスパイク）で、
他のSDRソフトと同じく「**信号からDCオフセットを除いて消す**」のが正攻法。捨てる（品質ゲートの
`dc-spike` 破棄）だけだと、検出帯の中心がズレた窓ではすり抜けて保存物を汚すことがあるため、
**除去を一次対策**に据える（品質ゲートの `dc-spike` は除去後の残り・例外に対する保険として残す）。

**方式（`dsp.remove_dc`）**: 受信機のDCは取得中に**時間変動**する（LO漏れの熱ドリフト）。
静的な複素平均（`iq - iq.mean()`）は **0Hz 成分しか消せない**ため、それだけだと中央に
**残留スパイク（実測 +4〜7dB）** が残る。そこで rate を渡し、**移動平均を引くハイパス**
（窓長 `win = rate / corner_hz`、既定 corner≈10kHz）で時間変動するDCにも追従して除去する。
コーナーは正準表現の周波数ビン幅（≈39kHz）より十分細かく取るので、中央に**スパイク（突出）も
ノッチ（へこみ）も残さず平坦**になる（実測で残留 +4〜7dB → **+0.7dB**）。中央外にオフセット
した信号・広帯域・バーストは触れないので本物は壊さない（実測でピーク Δ≈0dB）。
※ `rate` を渡さない `remove_dc(iq)` は静的平均のみの軽量版（0Hz成分だけ除去）。

**どこで効くか**: `sdr.HackRFBackend` が受信の**最も入口**（`_read`）で rate 付きで適用するため、
その後の全処理（サーベイ／ドウェル／測定／画像化／保存）が DC 除去済みの信号を見る。サーベイ
（`sweep_power`）も同じ入口を通るので、パワー計算で中央ビンが突出しない。継ぎ目
`capture_iq` / `sweep_power` のシグネチャは不変（戻り値が DC 除去済みになるだけ）。

**既存キャプチャの平坦化（表示時）**: すでに保存済みで中央に残留が見えるレコードは、
ビューアの `--flatten-dc` で表示時に追従ハイパスをかけて平坦化できる（**生IQには触れない**・
凍結 `spec.render` もそのまま）。新規取得は受信入口で平坦化済みなので不要。

```bash
python view_captures.py captures/ --flatten-dc        # 既存の中央DC残留を表示時に平坦化
```

```bash
# 実機: 既定で DC 除去が有効（中央スパイクは消えた状態で保存される）
python3 main.py --hardware --start 2.4e9 --stop 2.5e9 --dwell-seconds 10 --collect captures/

# 実機: DC 除去を無効化して挙動を比較したいとき
python3 main.py --hardware --no-dc-removal ...

# Sim 診断: 中央にDCスパイクを注入し（--sim-dc-spike）、--dc-removal で消えることを確認
python3 main.py --sim --dwell-seconds 2 --once --sim-dc-spike          # 注入のみ（中央に線）
python3 main.py --sim --dwell-seconds 2 --once --sim-dc-spike --dc-removal  # 除去され線が消える
```

- **既定**: 実機（`--hardware`）では**有効**（`SDRConfig.dc_removal=True`）。合成（`--sim`）は
  元々DCが無いので**既定オフ**（`--dc-removal` で強制有効化できる。`--no-dc-removal` が優先）。
- DC 除去を行ったかは保存する SigMF の `global` に `sigscan:dc_removed`（true/false）として
  正直に記録する（後段の再レンダ／学習時に判別できる）。
...
| `dc_removal` | True（実機） | 受信入口でDCオフセットを除去し中央スパイクを消す（`--no-dc-removal`／Sim診断は`--dc-removal`） |
...
| `dc_band_hz` / `dc_side_hz` | 60kHz / 0.8MHz | DCスパイク判定の中央バンド半幅／比較する両脇リング外縁 | — |
| `dc_excess_min_db` | 12dB | 中央が両脇よりこれ以上突出なら「中央集中」 | `--q-dc-excess` |
| `dc_excess_std_max` | 3dB | 中央集中度の観測間ばらつきがこれ以下なら「時間不変」 | `--q-dc-std` |
...
- **DCスパイク**は実機では受信入口の**DC除去（DC offset correction）が一次対策**で消す
  （[DCスパイク除去](#dcスパイク除去dc-offset-correction)）。品質ゲートの **DCスパイク除外**
  （`dc-spike`）は除去後の残り・例外に対する保険で、「中央集中・時間不変・細い」の3条件で判定する。
  逆に言えば、本物の狭帯域信号がたまたま取得帯域のちょうど中央に**常時一定**で居続けると、3条件を
  満たして DCスパイク扱いになりうる（本物のBLE等はホップ/明滅するか中央からオフセットするため
  通常は当たらない）。なお、滞在中ずっと帯域を埋め続ける常時広帯域信号は元々 dwell では過小検出
  （上記）であり、サーベイ側で拾う前提。
...
| `test_dsp.py`       | 測定        | `detect_segments` の帯検出、`measure_signal` の bw/SNR、`dc_spike_metrics` の中央集中度 |
| `test_dc_removal.py`| 取得（DC除去）| `remove_dc` の中央スパイク除去・本物信号(中央外/広帯域/バースト)非破壊、Sim注入spikeの除去、`build_backend` 配線、`sigscan:dc_removed` 記録 |
...
| `test_quality.py`   | 品質ゲート  | 単発/極細スプリアス/コムスプリアス/DCスパイクの破棄、正規狭帯域(BLE)・中央外・広帯域の保持、しきい値調整 |
