...
### 【重要な訂正・2026-07-15】バンド別ルーティングの配管は「未実装」
> 過去このファイルに「配管は実装済み（`config.py` の `BandPlan.cnn_model`、`classify.py` のルーティング分岐）」と書かれていたが、これは**誤り**（Claude の思い込み。`cnn_model` も `BandPlan` も全 `*.py` に grep 0 件・CC が2026-07-15確認）。実態は次の通り:
- CNN checkpoint は **単一グローバル**（`config.CNNConfig.checkpoint`、既定 `runs/m2_5`、`config.py:162-165`）。
- `scheduler` がそれを1度ロードし（`scheduler.py:88-96`）、**全バンドの検出に同じモデルを適用**（`scheduler.py:349-354`）。
- `config.Band`（`config.py:175-189`）のフィールドは name/f_lo/f_hi/priority/hint のみで、**モデル指定枠は無い**。
- ＝「専門家を登録したら 2.4GHz 帯で自動的に呼ばれる」は**現状できない**。バンド別ルーティングは**新規実装が必要**（専門家学習とは別タスク）。
- `BAND_PLAN`（`config.py`）は「1〜6GHz に何が存在するかの地図」であって「専門家の割り当て表」ではない（`BANDPLAN_PROPOSAL.md`「地図と踏破の分離」）。31帯域・最下限0.960GHz(DME/TACAN)。

### クラス体系（**案Z・Kali 承認済み 2026-07-15**）
2.4GHz ISM 専門家は **用途ベース**で始める。**unknown は教師0件のため、当面3クラスで開始**し、unknown は蓄積後に4クラス化:

| クラス | 現状（`method=human` 実測 2026-07-15） |
|---|---|
| `ble-adv` | **36件**（ch37/38/39 の3チャネル） |
| `wifi-24` | **23件** |
| `spurious` | **32件**（2440MHz 40MHz高調波） |
| `unknown` | **0件**（教師なし＝当面クラスから除外、後で追加） |
