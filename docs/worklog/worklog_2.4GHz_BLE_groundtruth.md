...
- **主目標**: BLE adv を各チャネルで human確定して ground truth を貯める。**3チャネル完成: ch38(2426) 3件・ch37(2402) 17件・ch39(2480) 15件 = BLE計35件（＋WiFi 2件）。全て method=human**。次は CNN 再学習に向けた他クラス（WiFi/Zigbee等）の蓄積。
...
## エントリ 2026-07-11: review_suggest を「視覚主・duty補助」に改修（持ち越し#5 解決）

### 狙い
Entry④で発覚した穴——13ms収集でdutyがinconclusiveになると、CCが `cc_class` を空欄のまま**全件skip**で提案し空振りする——を、**運用注意ではなくコードで構造的に塞ぐ**。主レシピが13msである以上、review_suggest はduty非依存で機能する必要がある。

### やったこと（CC丸投げ・指示書）
改修の芯は3点:
1. **recommend を `decide_recommend(spurious_warn, cc_class)` に集約し、duty を引数に取らない**（構造的にduty非依存を保証）。優先順: ①spurious_warn→skip（最優先ガード・不変）②cc_class 空/None→**needs-review** ③ble-adv→confirm-ble ④他→skip。
2. **cc_class 未記入を黙って通さない**。空欄は `needs-review`（第3の値）にフォールバックし、confirm_sheet冒頭と標準出力に件数付き警告。confirm_sheet を3節→**4節**化（Needs-review節を追加）。
3. **スプリアスガードは最優先で不変**（spurious_warn=True → skip 強制、cc_class を問わない）。

### 分かったこと / 結果
1. **持ち越し#5 解決**。194 → **196 passed / 3 skipped**（新規2・既存は意図保持の更新1）。凍結契約(spec/sigmf_io/main/observation) diff 空。変更は `review_suggest.py` とテストの2ファイルのみ。captures 非改変（mtime確認）、出力は `bench/` のみ。commit `c9d9fec`（push済み）。
2. **duty非依存が「構造」で保証された**。CCは `inspect.signature` で**引数にdutyが無いこと**をテストで固定＝運用注意でなく機械的にロック。「意志力なしで回る」の実装例。
3. **同一入力での回帰実証（前回の失敗が直った証拠）**。前回空振りした同じバッチ `2402MHz_178356*`（14件・13ms・duty全件inconclusive）で:
   - スケルトン段階: 旧来は「空欄→14件を黙ってskip」だったのが、今回は **needs-review=14 ＋ 警告発火**＝CCが視覚分類を忘れても**人間に必ず提示される**（穴が塞がった証拠①）。
   - CCが14枚を目視分類→全て `ble-adv` 記入→併合: **confirm-ble=14 / skip=0 / needs-review=0 / warnings=0**。**前回=全skip空振り → 今回=全confirm-ble**。dutyは14件とも inconclusive のまま＝**recommendがduty非依存で機能した証拠②**。

### CCの実力について（4点目のデータ）
- **13ms・duty inconclusive下でも、CCの視覚提案が機能する**ことが初めて実データで確認された（PNG14枚→14/14 ble-adv、人間の確定結果と一致）。前回は工程を飛ばして測れなかった項目。
- ただし例によって**明快なBLEケース**であり、持ち越し#2（人間 vs CC の判別力）は依然未測定。

### この日の成果物
- `cnntrain/review_suggest.py` 改修（`decide_recommend` / needs-review / 4節シート / バナー5点目）、`tests/test_review_suggest.py`（+2・既存1更新）。commit `c9d9fec`。
- `bench/review_suggest_ch37_b2/`（回帰実証の出力）＝サンドボックス。
- **ground truth の追加なし**（今回は道具の改修セッション）。

---

## エントリ 2026-07-13: ch39(2480) 達成＝3チャネル完成 ＋ 確定フロー自動化（○×UI・訂正経路）

### 狙い
持ち越し#3（ch39収集）。2480は40MHzクロック高調波のド真ん中なので `--dwell-offset-hz 4e6` の初実戦になる想定だった。前回の教訓に従い、**まず対照ラン（ゲート無効）で「2480に何が居るか」を見てから**作戦を決める方針。

### 分かったこと / 結果

**1. オフセットは不要だった（持ち越し#3の前提が実測で覆った）**
対照ラン（13ms・ゲート無効・60件）のPNGを見ると、**2480にBLE ch39 advの離散バーストが明快に立っている**（det 2480.0・BW1.2-1.3MHz・spur_suspect=False）。一方、予想していた**40MHz高調波の定常線は2480ではなく2484近傍**に出ていた。つまり**BLEとスプリアスが重なっておらず、オフセットで避ける必要がない**。むしろ tuner 2480.0（＝オンDC）で最も明快なバースト（SNR36.8dB）が撮れた＝**dc-spike説の「白」がさらに補強**。`--dwell-offset-hz` は今回不発（将来別用途で）。
- mean spectrum の 2480 ピークは**間欠バーストの時間平均**（毎回同じ2480に出るから山になる）でスプリアスではない。スペクトログラムに定常横線が無いことで判別できた。

**2. persist が初めて連続分布した（間欠BLEの証拠）**
対照ランの persist は **0.22〜1.00 に連続的にばらけた**（ch37の13msでは1.00に貼り付いていた）。低persist（0.22-0.47）は「19観測窓のうち4-9窓にしか出ない」＝**明確な間欠**。BLE advを正しく捉えている強い証拠。→ `--q-min-persistence 0.2` が必須（既定0.34だと低persistのBLEを落とす）。

**3. 本収集24件・narrow-steady-spur が1件も発火せず**
13ms・ゲートON・persist0.2 で本収集 → **`drop:narrow-steady-spur` がゼロ**。BLEのpersistが低くBW1.1-1.6MHzなので「狭帯域かつ定常」に噛まない。ゲートは正しく振る舞っている（Entry②の窓長判断が再現）。

**4. 人間が手順を飛ばして誤確定（重要な失敗）**
review_suggest を回した直後、**CCの視覚分類を待たずに** review.py で16件全部を `5`(BLE) で確定してしまった。結果、`2480MHz_1783755115496_11`（det 2480.81・外れ値）を**BLEと誤確定**。実際は検出帯の主役が**上下貫通のWiFi縦帯**で、BLEブロブは帯域外(下)。
- **ツールは正しく警告していた**（confirm_sheet が `needs-review=16`＋警告を出していた）が、**人間がそれを無視して確定した**。ツールが正しくても運用でスキップされうる、という教訓。

**5. 訂正経路が無い＝誤ラベルが恒久固定される穴が露呈**
`_11` を直そうとしたが、`review.py --pattern` で **0件**。原因は `find_low_confidence()` が `method=='rule'` だけを拾う仕様で、**一度 human確定したレコードは二度と呼び戻せない**。人間が間違える以上、訂正経路は必須。

### やったこと（CC丸投げ・3点セット）
1. **バックグラウンド化**: review_suggest に `--auto-classify`。CCが収集済みバッチのPNGを自動で視覚分類し、confirm_sheet まで一気に到達（人間の手作業は「シートを見る」だけ）。confirm_sheet に**PNGパス列**を追加。
2. **○×UI**: review.py に `--suggest`。**CC提案＋根拠＋客観指標＋PNGパス**を提示し **y/n** で確定。`n` のときだけラベル一覧。
   - **摩擦（安全弁）＝Pattern A化の構造的防止**: `cc_class` が `unclear` / `spurious_warn=True` / `needs-review`(未記入) のいずれかなら **`y` を提示せず、ラベル一覧を強制表示**。「y連打で素通り＝AI出力が実質そのまま ground truth になる」を不能化。テスト3本でロック。
3. **訂正経路**: review.py に `--include-human`。human確定済みも対象にでき、**訂正時は `sigscan:relabel_history` に `{from,to,at}` を append**（黙って上書きしない）。

**結果**: 196 → **211 passed / 3 skipped**（+15、既存1件も改変せず）。凍結契約 diff 空。commit `baf43e5`（push済み）。

### CCの実力について（5点目のデータ・初の"人間超え"事例）
- **`_11` をCCが独立に `wifi` と判定**。根拠も精緻:「検出帯(2480.8)の主役は t≈0.28 の上下貫通WiFi縦帯(20MHz級)・BLEブロブ(t≈0.08, 2480.0)は帯域外(下)」。Claudeが画像から読んだ結論と一致し、**人間（Kali）の誤確定を正した**。
- 各件の根拠を見ると、CCは**「検出帯の主役 vs 通過事象/帯域外」を一貫して区別**している（例:「t≈0.73のWiFi縦帯は帯域を貫く通過事象」「t≈0.10のブロブ(2477.5)は帯域外」）。視覚判断の質は高い。
- **ただし能力差の証明ではない**: 人間側は「手順を飛ばして急いだ」状況だった。差が出たのは能力でなく**手順の有無**。まさに今回の摩擦実装が防ごうとしたもの。持ち越し#2（判別力ベンチ）は依然として別途必要。

### この日の成果物
- **ground truth**: BLE ch39 adv **15件** + WiFi 1件（`_11` 訂正）method=human。**3チャネル完成・BLE計35件**。
- `review.py`（`--suggest` ○×UI・`--include-human` 訂正経路・履歴）、`cnntrain/review_suggest.py`（`--auto-classify`・PNGパス列）、`tests/test_review_confirm.py`(+15)。commit `baf43e5`。
- `bench/review_suggest_ch39_v2/`（CC提案・confirm-ble=15 / skip=1(`_11` wifi)）。
...
3. ~~**ch39(2480)収集**~~ → **【解決 2026-07-13】** BLE adv 15件 human確定（3チャネル完成）。**`--dwell-offset-hz 4e6` は不要だった**——2480の40MHz高調波はBLEと重ならず**2484近傍**に居たため、オンDC(tuner 2480)でそのまま撮れた（dc-spike白の補強）。オフセットは今回不発で、将来別用途向け。詳細は Entry 2026-07-13。
4. **dutyprobe の位置づけ**: あくまで測定（時間占有＝burst/continuous）。**WiFiとBLEは両方低dutyで分離不能**（Entry③実証）＝信号種の同定はしない。BLE/非BLEラベルにも教師にも使わない。
5. ~~**review_suggest の inconclusive 対応**~~ → **【解決 2026-07-11 / commit c9d9fec】** `decide_recommend(spurious_warn, cc_class)` に集約し **duty を引数から除去**（signature検査でロック）。cc_class 空欄は黙ってskipせず **needs-review**＋警告。スプリアスガードは最優先で不変。196 passed。同バッチ `2402MHz_178356*` で**前回=全skip空振り → 今回=confirm-ble 14件**（duty inconclusive のまま機能）を実証。詳細は Entry 2026-07-11。
...
7. **`run_suggest_review` が `--pattern` / `--include-human` を無視する**（Entry 2026-07-13で露呈・要修正）。`_11` 1件だけを狙って `--pattern "2480MHz_1783755115496*"` を付けたのに **16件全部が列挙された**。原因は `--suggest` のCSVをそのまま対象集合にしていて、pattern でフィルタしていないため。実害は軽微だった（他15件は BLE→BLE の同一ラベル再確定で、`relabel_history` に無意味な履歴が15件入っただけ）が、**「狙った1件だけを訂正する」という訂正経路の要件を満たしていない**。次の改修で `run_suggest_review` も pattern/include_human でフィルタするよう直す。
8. **CNN再学習の準備**（BLE 35件貯まった＝そろそろ着手圏）。BLE adv は3チャネル計35件のhuman確定が揃った。再学習には他クラス（WiFi 2.4GHz / Zigbee / 未識別・スプリアス等）も各数十件のhuman確定が要る。次の収集メニューはWiFi 2.4GHz（wideband-ofdmクラス）。現CNNは合成100%学習で実RFを一度も見ていないため、HackRF固有スプリアスを高確信で誤る——これは実測ground truthでしか直らない（持ち越し#6の落とし穴1）。
9. **confirm_sheet の見出しが常に "ch37" 固定**（CCが正直に報告した軽微な表示バグ）。ch39バッチでも "確定シート: BLE ch37 レビュー提案" と出る。表示のみで実害なし。次のついでに動的化。
...
- **review_suggest は視覚主・duty補助**（Entry④で判明 → **2026-07-11 実装済み c9d9fec**）：13ms主レシピではdutyがinconclusiveになるので、提案の主判定はCCの視覚＋メタ＋スプリアスガード、dutyは補助列。**穴は運用注意でなくコードで塞ぐ**——`decide_recommend` は duty を引数に取らず（signature検査でロック）、cc_class 未記入は黙ってskipせず needs-review で人間に必ず提示。「意志力なしで回る」の実装例。
- **確定は○×UIで（提案＋根拠＋客観＋PNGパスを見て y/n）**（2026-07-13実装 `baf43e5`）：大半は y を押すだけ、危ういものだけ手を動かす。ただし**摩擦を必ず残す**——`unclear` / `spurious_warn` / `needs-review` では **y を提示せず強制ラベル選択**。これが無いと「y連打＝AI出力がそのまま ground truth」＝**Pattern A化**する。利便性と思想の両立点。
- **人間も間違える → 訂正経路は必須**（2026-07-13）：`--include-human` で human確定済みを呼び戻し、`relabel_history` に `{from,to,at}` を残して訂正する。**誤ラベルの恒久固定を防ぐ**。実際に `_11`（BLE誤確定→WiFi）を訂正した。
- **ツールが正しく警告しても、人間が手順を飛ばせば汚染する**（2026-07-13の失敗）：confirm_sheet は `needs-review=16` と警告していたのに、人間がそれを無視して16件を一括確定し `_11` を誤ラベルした。**手順を飛ばせない構造**（○×UI・摩擦）が要る理由。
